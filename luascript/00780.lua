...
function houselearn_30_1()
	...
	local bookdbid = 202730		--/*此處設定書的物件編號
		...
		if ZoneID == 0 then
			if ( SkillLV > lvb or SkillLV1 > lvb or SkillLV2 > lvb or SkillLV3 > lvb or SkillLV4 > lvb or SkillLV5 > lvb and SkillLV <= lva and SkillLV1 <= lva and SkillLV2 <= lva and SkillLV3 <= lva and SkillLV4 <= lva and SkillLV5 <= lva) then--/*判斷等級
				if CountBodyItem( OwnerID() , bookdbid ) >= 1 then			--/* 判斷是否有書
					BeginCastBar( OwnerID(), GetString("SC_BOOKCAST_30") , truecast , ruFUSION_ACTORSTATE_SIT_BEGIN , ruFUSION_ACTORSTATE_SIT_END , 0 );	--/*開始施法
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------製造百科全書大全II
function houselearn_31()
	BeginPlot(OwnerID() , "houselearn_31_1" , 0 )
end
function houselearn_31_1()
	local average = 3			--/*此處設定給予的經驗值
	local bookdbid = 202731		--/*此處設定書的物件編號
	...
	local SkillLV5 = ReadRoleValue(OwnerID(), EM_RoleValue_Skill_Alchemy)
		if ZoneID == 0 then
			if ( SkillLV > lvb or SkillLV1 > lvb or SkillLV2 > lvb or SkillLV3 > lvb or SkillLV4 > lvb or SkillLV5 > lvb and SkillLV <= lva and SkillLV1 <= lva and SkillLV2 <= lva and SkillLV3 <= lva and SkillLV4 <= lva and SkillLV5 <= lva) then--/*判斷等級
				if CountBodyItem( OwnerID() , bookdbid ) >= 1 then			--/* 判斷是否有書
					BeginCastBar( OwnerID(), GetString("SC_BOOKCAST_31") , truecast , ruFUSION_ACTORSTATE_SIT_BEGIN , ruFUSION_ACTORSTATE_SIT_END , 0 );	--/*開始施法
					Castspell(OwnerID(),OwnerID(),491282)					--/*在手上加書
					while 1 do
						sleep( 2 );
						local CastBarStatus = CheckCastBar( OwnerID() );
