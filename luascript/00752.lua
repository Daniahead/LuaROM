...
-----------------------------
	...
	if CheckFlag( OwnerID() , 541579 ) and CheckFlag( OwnerID() , 541580 ) and CheckFlag( OwnerID() , 541581 ) then --是不是完成三次傳遞
		...
		ShowBorder( OwnerID(), 420883, GetObjNameByGUID(420883), "ScriptBorder_Texture_Paper" )
		return
	end
	...
	if NowZ~=0 and NowN ~=0 then
		...
		ShowBorder( OwnerID(), QuestID, GetObjNameByGUID(QuestID), "ScriptBorder_Texture_Paper" )
		return
	end
	--直接用rand的去洗出未接過的
	while true do
