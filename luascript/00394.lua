...
--	DisableQuest( OwnerID() , true )
	...
	for i = 1 , 3 , 1 do
		Spider[i] = CreateObjByFlag ( 100030 , 580044, i ,1 )
...
function LuaQ_420088_Play()
	local PlayerTeam = LuaFunc_PartyData( TargetID() )
	for i = 1 , PlayerTeam[0] , 1 do
		if CheckCompleteQuest(PlayerTeam[i] ,420083) == false and CountBodyItem( PlayerTeam[i] , 200191 ) == 0 then
			GiveBodyItem( PlayerTeam[i] , 200191 , 1 )
		end
	end 
	local Rog = CreateObjByFlag ( 110501 , 580044, 0 ,1 )
...
function LuaQ_420088_Play_0()
	...
	sleep( 30 )
	local Spear = LuaFunc_CreateObjByObj ( 110492 , MagicPlot )
...
function LuaS_200191_0() --羅格的骨骸
	...
	ShowBorder( OwnerID(), 420083, GetObjNameByGUID(200191), "ScriptBorder_Texture_Paper" );	
