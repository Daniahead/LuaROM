...
function LuaFunc_CreateNPCByMatrix(ObjID,Matrix,A,B ) --在固定座標點產生物件的函式
	...
	WriteRoleValue( Obj , EM_RoleValue_Money , A ) --紀錄A的位置，A必定大於0，因此可用錢幣來記錄
...
function LuaFunc_ObjMatrix_Record(ObjMatrix)--紀錄原先NPC身上的資料，以供重生使用
	...
	for pry , obj in ObjMatrix do 
		if obj ~= nil and CheckID(obj) == true then
			...
			ObjRecord["A"][pry] = ReadRoleValue( obj , EM_RoleValue_Money )
...
function LuaFunc_ResetMatrix( ReviveTime , Matrix)
	if ReviveTime < 0 or Type(Revivetime) ~= "number" then
		Revivtime = ReSetNPCInfo( OwnerID() ) --輸入 0 以下或是非數字的重生時間，就參考NPC本身的重生時間進行重生
	else
		ReviveTime = math.ceil( ReviveTime ) --無條件進位強制 ReviveTime 為整數
	end
