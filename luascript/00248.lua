...
function LuaS_DEGRADE_MINING() --挖礦用
	local NPC = ReadRoleValue( TargetID() , EM_RoleValue_OrgID)
		...
		if(ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Mining ) == 80) then
...
function LuaS_DEGRADE_LUMBERING() --伐木用
	local NPC = ReadRoleValue( TargetID() , EM_RoleValue_OrgID)
		...
		if(ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Lumbering ) == 80) then
...
function LuaS_DEGRADE_HERBLISM() --採藥用
	local NPC = ReadRoleValue( TargetID() , EM_RoleValue_OrgID)
		...
		if(ReadRoleValue ( OwnerID(), EM_RoleValue_SkillMax_Herblism ) == 80) then
