	...
	if CheckFlag( OwnerID()  , 540778 ) == true then
		...
		if CheckAcceptQuest( OwnerID() , 420481) == true then
			if CountBodyItem( OwnerID() , 201190 ) >= 1 then
				BeginCastBar( OwnerID(), GetString("CASTBAR_NAME_01") , 30 , ruFUSION_ACTORSTATE_CRAFTING_BEGIN , ruFUSION_ACTORSTATE_CRAFTING_END , 0 );	-- 畫下巨石上的符文標誌
				while 1 do
					sleep( 2 );
		...
		elseif CheckAcceptQuest( OwnerID() , 420479) == true then
			if CountBodyItem( OwnerID() , 201192 ) == 0 then		--/* 判斷是否有六顆狼心	--/* 判斷是否有魔物之血
				if CountBodyItem( OwnerID() , 201189 ) >= 10 then
					BeginCastBar( OwnerID(), GetString("CASTBAR_NAME_02") , 30 , ruFUSION_ACTORSTATE_CRAFTING_BEGIN , ruFUSION_ACTORSTATE_CRAFTING_END , 0 );	-- 召喚魔物
					while 1 do
						sleep( 2 );
						local CastBarStatus = CheckCastBar( OwnerID() );
					...
					if ( resultM == "OKOK" ) then
						...
						local Bear = CreateObj(100560, BaseX, BaseY, BaseZ, BaseDir, 1 )	
		...
		elseif CheckAcceptQuest( OwnerID() , 420480) == true then
			if  CountBodyItem( OwnerID() , 201193 ) == 1 then
				BeginCastBar( OwnerID(), GetString("CASTBAR_NAME_03")  , 60 , ruFUSION_ACTORSTATE_CRAFTING_BEGIN , ruFUSION_ACTORSTATE_CRAFTING_END , 0 );	-- 封印魔法陣
				while 1 do
					sleep( 2 );
					local CastBarStatus = CheckCastBar( OwnerID() );
