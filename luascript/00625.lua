	...
	while true do
		...
		if (  AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) == true ) then  -- 檢查是否離開戰鬥
			...
			if ( HPPercent <= 0.10 ) and ( HelpAttack == "false" ) then
				...
				if ( RND <= 20 ) then
					local HelpAttack , HelpMonster = H_K_CallHelpMonster( OwnerID() , HelpMonsterID , HelpRange , GetString("MT_KAMOKU2") )
					--SetAttack( HelpMonster , AttackTarget )
					Move( HelpMonster , X , Y , Z )
				end
			end
			...
			for i = 1, table.getn( Monster1 ) do	-- 離開戰鬥疾影
				if ( ( Monster1[i] ~= nil ) and ( CheckID( Monster1[i] ) == true ) ) then		DelObj( Monster1[i] )	Sleep( 1 )		Monster1 = {}	end
			end
			for i = 1, table.getn( Monster2 ) do	-- 離開戰鬥疾影
				if ( ( Monster2[i] ~= nil ) and ( CheckID( Monster2[i] ) == true ) ) then		DelObj( Monster2[i] )	Sleep( 1 )		Monster2 = {}	end
			end
			for i = 1, table.getn( Monster3 ) do	-- 離開戰鬥疾影
				if ( ( Monster3[i] ~= nil ) and ( CheckID( Monster3[i] ) == true ) ) then		DelObj( Monster3[i] )	Sleep( 1 )		Monster3 = {}	end
			end
		end
	...
	while true do
		...
		if (  AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) == true ) then  -- 檢查是否離開戰鬥
			...
			if ( HPPercent <= 0.10 ) and ( HelpAttack == "false" ) then
				...
				local RND =  Rand( 100 ) + 1
				if ( RND <= 20 ) then
					local HelpAttack , HelpMonster = H_K_CallHelpMonster( OwnerID() , HelpMonsterID , HelpRange , GetString("MT_HERDOLEN2") )
					--SetAttack( HelpMonster , AttackTarget )
					Move( HelpMonster , X , Y , Z )
				end
			end
...
function H_K_CallHelpMonster( Player , HelpMonsterID , HelpRange , SayString )
	...
	local X = ReadRoleValue(  Player , EM_RoleValue_X - 75 )
	local Y = ReadRoleValue(  Player , EM_RoleValue_Y )
	local Z = ReadRoleValue(  Player , EM_RoleValue_Z - 75 )
	local HelpAttack = "false"
