function LuaI_420226_0()
	...
	while 1 do
		...
		local Trace = CreateObjByFlag( 110663 , 580023 , StepPrint  , 1 )		-- (產生物件ID , 產生旗子ID , 旗子位置ID , 產生數量)
...
function LuaQ_420226_Item() --使用道具
	...
	if CheckCompleteQuest( OwnerID() ,420226 ) then
		...
		if Check == 2 and CountBodyItem( OwnerID() , 200677 ) == 0 then
			...
			sleep ( 30 )
			local Goblin = CreateObjByFlag( 100098 , 580023 , Step ,1 ) --代100098
			Func_rand_walker(Goblin,300, 10 ) --讓怪物不會出現在腳印上
			sleep(30)
...
function LuaS_200677_0() --奇異的藥劑
	...
	ShowBorder( OwnerID(), 420223, GetObjNameByGUID(GetUseItemGUID(OwnerID())), "ScriptBorder_Texture_Paper" );	
