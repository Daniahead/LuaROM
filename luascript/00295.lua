...
function LuaQ_420263_Dead()
	...
	if Ver_Zone103_Trap[RoomID]["DRAMA"] == 1 then
		Say( OwnerID() ,GetString("SAY_100178_3") ) --/*這……不可能……我的力量……
	else
		Say( OwnerID() ,GetString("SAY_100178_4") ) --/*別得意......看看你們回去的路吧！我留了一個禮物給你們......哈哈哈！
		local COGD = CreateObjByFlag( 100274 , 580030 , 4 , 1 ) --產生屍骸奧古達
...
--	Hide( Cage )
	sleep( 20 )
	local Guard = {}
	--580030旗標在此回收利用，與副本中所代表的意義不同
	LuaFunc_MoveToFlag( OwnerID() , 580030 , 1 , 0 ) --有轉彎
	AdjustFaceDir( OwnerID(), SandGlass, 0 )
	sleep( 30 )
	Say( OwnerID() , GetString("SAY_110251_0") ) --/*哇～如果可以在裡面研究一定很棒！
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 30 )
	AdjustDir( OwnerID(), 180 ) --接轉身180度的動作
	sleep( 30 )
	Say( OwnerID() , GetString("SAY_110251_1") ) --/*不過我得趕快回去把這些重要的資料給記下來。 
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 30 )
	Say( OwnerID() , GetString("SAY_110251_2") ) --/*走吧！愈快愈好！我把資料整理好之後還要趕回這裡繼續研究呢！
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 5 )
	...
	Say( OwnerID() , GetString("SAY_110251_3") ) --/*喔！是援軍耶！
	WriteRoleValue( OwnerID() ,EM_RoleValue_IsWalk , 0  )
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 5 )
	LuaFunc_MoveToFlag( OwnerID() , 580030 , 0 , 0 )
	Say( Guard[5] ,GetString("SAY_110251_4")) --/*謝謝你的協助，夏麥爾隊長派我們來接應你們，接下來就交給我們吧！ 
	PlayMotion( Guard[5]  , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 5 )
	Say( OwnerID() ,GetString("SAY_110251_5")) --/*太好了！快走吧！我受不了這些慢吞吞的傢伙了！
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 30 )
	Say( Guard[5] ,GetString("SAY_110251_6")) --/*<玩家姓名>大人，夏麥爾隊長希望您在回到哈洛夫之時能夠找他談談，他希望能當面感謝您的幫助。
	PlayMotion( Guard[5]  , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 5 )
	Say( OwnerID() ,GetString("SAY_110251_7")) --/*拖拖拉拉做什麼？我還趕著要把這些資料給記下來呢！紙，你們有帶紙嗎？
	PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
	sleep( 30 )
	Say( Guard[5] ,GetString("SAY_110251_8")) --/*那我們先告辭了。
	...
	for i = 1 , 5 , 1 do 
		...
		if i == 5 then
			Say( OwnerID() ,GetString("SAY_110251_9")) --/*我們回去整理完這些資料之後，還要準備回來這裡研究……
			PlayMotion( OwnerID() , ruFUSION_ACTORSTATE_EMOTE_SPEAK)
			sleep( 10 )
			Say( Guard[i] ,GetString("SAY_110251_10")) --/*抱歉，我們在這個任務之後還有其他勤務要執行。
