	...
	while true do
		...
		if (  AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) == true ) then  -- 檢查是否離開戰鬥
			...
			for i = 1, table.getn( Monster ) do
				if ( ( Monster[i] ~= nil ) and ( CheckID( Monster[i] ) == true ) ) then	DelObj( Monster[i] )	Sleep( 1 )		Monster = {}	end
			end
		end
	...
	while true do
		...
		if (  AttackTarget ~= 0 ) and ( CheckID( OwnerID() ) == true ) then  -- 檢查是否離開戰鬥
			...
			for i = 1, table.getn( Monster1 ) do	-- 離開戰鬥時，刪除進入戰鬥時叫出來的影子
				if ( ( Monster1[i] ~= nil ) and ( CheckID( Monster1[i] ) == true ) ) then		DelObj( Monster1[i] )	Sleep( 1 )		end
			end
			for i = 1, table.getn( Monster2 ) do	-- 離開戰鬥時，刪除進入戰鬥時叫出來的影子
				if ( ( Monster2[i] ~= nil ) and ( CheckID( Monster2[i] ) == true ) ) then		DelObj( Monster2[i] )	Sleep( 1 )		end
			end

			for i = 1, table.getn( Monster3 ) do	-- 離開戰鬥時，刪除進入戰鬥時叫出來的影子
				if ( ( Monster3[i] ~= nil ) and ( CheckID( Monster3[i] ) == true ) ) then		DelObj( Monster3[i] )	Sleep( 1 )		end
			end
		end
