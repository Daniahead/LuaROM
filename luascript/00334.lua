...
function LuaQ_420346_StepA()
	Hide( OwnerID() )
	local Conniver = CreateObjByFlag(100424,580039, 0 , 0 )
...
function LuaQ_420346_StepC()
	local OBMT = CreateObjByFlag(100423,580039, 0 , 0 )
	local Paper = ReadRoleValue( TargetID() , EM_RoleValue_PID )
	SetRoleCamp(OBMT,"NPC")
...
function LuaQ_420346_StepD()
	...
	LuaFunc_MoveToFlag( OwnerID(), 580039 , 1 ,0 )
				...
				if i ~= 12 then
					...
					for j = 2 , 4 , 1 do
						if CheckID( Guard[i] ) == false then
							Guard[i] = CreateObjByFlag( 100421 , 580039 , j , 1 )
