function LuaI_110041()
	...
	local RFG = CreateObjByFlag( 100195 , 580036 , 2 , 1 ) --代用
...
function LuaI_RFG_SHOW() --瑞芬格的出現劇情
	...
	SetPosByFlag( OwnerID(), 580036 , rand )
...
function LuaQ_420296_ATTACK()
	...
	local Head = CreateObjByFlag( 110261 , 580036 , 0, 1 ) --代用
	local RFG_B= CreateObjByFlag( 100197 , 580036 , 1 , 1 )
...
Function LuaQ_420296_Complete()
	...
	local Head = CreateObjByFlag( 110261 , 580036 , 11 , 1 )
	AddToPartition( Head, 0 ) 
	sleep( 30 )
...
--	ScriptMessage( OwnerID(), TargetID(), 0, GetString("EM_420296_3"), 0 ); --/*疤眼對瑞芬格的頭顱吐口水。
	...
	DelObj(Head)
	local Corpse = CreateObjByFlag( 110272 , 580036 , 11 , 1 )
	...
	LuaFunc_MoveToFlag( OwnerID(), 580036 , 12 ,0 )
...
--	CastSpell( Corpse , Corpse , xxxxxx ) --瑞芬格出現的特效
	...
	SetPosByFlag( OwnerID(), 580036 , 10 )
	Show( OwnerID() , 0 )
end