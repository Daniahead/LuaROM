...
function LuaS_110773_HomeBUY_Dialog()
	...
	local Ret = BuildHouse( TargetID() , 0 );
	if( Ret == false ) then
	...
	if ( homenumber ~= -1 ) then
		...
		Tell( TargetID() , OwnerID() , GetString("SO_GIVEHOMEOKMAID")..homenumber_text )	-- 申請已經完成，主人你的房屋門牌號碼是
	else
...
function LuaS_HomeFT_Give()
	...
	elseif ( RND2 > 95 and RND2 <= 100 ) then
		...
		Tell( OwnerID() , TargetID() , GetString("SO_HOMEFT_STR1")..FT1..GetString("SO_HOMEFT_STR2")..FT2..GetString("SO_HOMEFT_STR4") )
end
